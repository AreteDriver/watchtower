...
export function SystemDossier() {
  ...
  return (
    <div className="space-y-6">
      ...
      {(() => {
        const hasTier = subscription && subscription.active && subscription.tier >= 1;
        const loadNarrative = () => {
          if (!systemId) return;
          setNarrativeLoading(true);
          api.systemNarrative(systemId)
            .then((d) => setNarrative(d.narrative))
            .catch(() => setNarrative('Briefing unavailable. Ensure you have Scout tier or above.'))
        ...
        return (
          <div className="bg-[var(--eve-surface)] border border-[var(--eve-border)] rounded-lg p-4 flex items-center justify-between">
            <div>
              <h3 className="font-mono text-[10px] uppercase tracking-[0.2em] text-[var(--eve-orange)] font-bold">
                Intelligence Briefing
              </h3>
              <p className="text-xs text-[var(--eve-dim)] mt-1">
                {hasTier ? 'AI-generated system analysis from on-chain data' : 'Requires Scout tier or above'}
              </p>
            </div>
            <button
              ...
              className={`px-3 py-1.5 rounded text-xs font-mono ${
                hasTier
                  ? 'bg-[var(--eve-green)]/20 text-[var(--eve-green)] border border-[var(--eve-green)]/40 hover:bg-[var(--eve-green)]/30'
                  : 'bg-[var(--eve-surface)] text-[var(--eve-dim)] border border-[var(--eve-border)] cursor-not-allowed'
              }`}
            >
              Generate Briefing
            </button>
