...
describe("api", () => {
  ...
  it("entity calls correct endpoint", async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith("/api/entity/xyz");
  });

  it("feed passes limit parameter", async () => {
    mockOk({ items: [] });
    await api.feed(10);
    expect(mockFetch).toHaveBeenCalledWith("/api/feed?limit=10");
  });

  it("feed uses default limit of 20", async () => {
    mockOk({ items: [] });
    await api.feed();
    expect(mockFetch).toHaveBeenCalledWith("/api/feed?limit=20");
  });
  ...
  it("narrative calls correct endpoint", async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith("/api/entity/x/narrative");
  });

  it("reputation calls correct endpoint", async () => {
    mockOk({ entity_id: "x", trust_score: 50 });
    await api.reputation("x");
    expect(mockFetch).toHaveBeenCalledWith("/api/entity/x/reputation");
  });

  it("leaderboard calls correct endpoint", async () => {
    mockOk({ entries: [] });
    await api.leaderboard("kills");
    expect(mockFetch).toHaveBeenCalledWith("/api/leaderboard/kills");
  });
  ...
  it("entities without type filter", async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith("/api/entities?limit=10");
  });

  it("subscription calls correct endpoint", async () => {
    mockOk({ wallet: "0xabc", tier: 1 });
    await api.subscription("0xabc");
    expect(mockFetch).toHaveBeenCalledWith("/api/subscription/0xabc");
  });

  it("streak calls correct endpoint", async () => {
    mockOk({ entity_id: "x", current_streak: 5 });
    await api.streak("x");
    expect(mockFetch).toHaveBeenCalledWith("/api/entity/x/streak");
  });
});
